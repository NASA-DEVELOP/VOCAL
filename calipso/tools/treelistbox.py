...
class TreeListBox(object):
    ...
    def __init__(self, root, headers):
        ...
        y_scrollbar = Scrollbar(self.__root, orient=VERTICAL, command=self.tree.yview)
        x_scrollbar = Scrollbar(self.__root, orient=HORIZONTAL, command=self.tree.xview)
        self.tree.configure(yscrollcommand=y_scrollbar.set, xscrollcommand=x_scrollbar.set)
        y_scrollbar.pack(side=RIGHT, fill=Y, expand=NO)
        x_scrollbar.pack(side=BOTTOM, fill=X, expand=NO)
...
def sort_by(tree, col, descending):
    ...
    tree.heading(col, command=lambda cl=col: sort_by(tree, cl, int(not descending)))